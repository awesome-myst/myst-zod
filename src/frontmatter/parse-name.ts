...
// Vendored from myst-frontmatter
...
export function parseName(literal: string): Name {
  ...
  if (displayParts.length === 1) {
    return {
      literal,
      ...givenAndParticle,
      ...parseParticleFamily(displayParts[0]),
    };
  }
...
function parseParticleFamily(name: string): Name {
  ...
  while (
    nameParts.length &&
    startsWithUpperCase(nameParts[nameParts.length - 1])
  ) {
...
export function formatName(name: Name, alwaysReversed = false): string {
  ...
  if (!alwaysReversed && !hasCommas && !dropping_particle && !suffix) {
    const formattedName = [given, non_dropping_particle, family]
      .filter(Boolean)
      .join(" ");
